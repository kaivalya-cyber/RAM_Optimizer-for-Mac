...
class RAMOptimizerDashboard:
    ...
    def view_logs(parent_root):
        ...
        # Title
        title = tk.Label(
            log_window, text="📋 Optimization Action Logs", font=("Helvetica", 18, "bold"), bg="#1e1e1e", fg="#00ff00"
        )
        title.pack(pady=15)
        ...
        log_text = tk.Text(
            text_frame, bg="#2d2d2d", fg="#cccccc", font=("Courier", 11), wrap=tk.NONE, insertbackground="white"
        )
    ...
    def create_gui(self):
        ...
        title_label = tk.Label(
            self.root, text="🚀 RAM Optimizer Dashboard", font=("Helvetica", 24, "bold"), bg="#1e1e1e", fg="#00ff00"
        )
        ...
        stats_frame = tk.LabelFrame(
            main_frame, text="Memory Statistics", font=("Helvetica", 14, "bold"), bg="#2d2d2d", fg="#ffffff"
        )
        ...
        pressure_frame = tk.LabelFrame(
            main_frame, text="Memory Pressure", font=("Helvetica", 14, "bold"), bg="#2d2d2d", fg="#ffffff"
        )
        ...
        self.pressure_label = tk.Label(
            pressure_frame, text="Normal", font=("Helvetica", 12, "bold"), bg="#2d2d2d", fg="#00ff00"
        )
        ...
        style.configure("Treeview.Heading", background="#2d2d2d", foreground="white", font=("Helvetica", 11, "bold"))
        ...
        control_frame = tk.LabelFrame(
            main_frame, text="Optimization Controls", font=("Helvetica", 14, "bold"), bg="#2d2d2d", fg="#ffffff"
        )
        ...
        threshold_label = tk.Label(auto_frame, text="%", font=("Helvetica", 11), bg="#2d2d2d", fg="white")
        ...
        alert_pct_label = tk.Label(alert_frame, text="%", font=("Helvetica", 11), bg="#2d2d2d", fg="white")
        ...
        self.status_label = tk.Label(
            self.root, text="Ready", font=("Helvetica", 10), bg="#1e1e1e", fg="#888888", relief=tk.SUNKEN
        )
    ...
    def save_settings(self):
        ...
        settings = {
            ...
            "auto_optimize_threshold": (
                self.auto_optimize_threshold_var.get() if hasattr(self, "auto_optimize_threshold_var") else 85.0
            ),
    ...
    def create_stat_label(self, parent, text, row):
        ...
        label = tk.Label(frame, text=text, font=("Helvetica", 11), bg="#2d2d2d", fg="#aaaaaa", width=20, anchor="w")
        label.pack(side=tk.LEFT)
        self._register_widget(label, {"bg": "#2d2d2d", "fg": "#aaaaaa"}, {"bg": "#ffffff", "fg": "#666666"})

        value_label = tk.Label(frame, text="---", font=("Helvetica", 11, "bold"), bg="#2d2d2d", fg="#00ff00")
    ...
    def update_graph(self):
        ...
        if len(self.timestamps) > 0:
            ...
            labels = [
                self.timestamps[i] if i < len(self.timestamps) else "" for i in range(0, len(self.timestamps), 10)
            ]
            self.ax.set_xticklabels(labels, rotation=45, color="white")
    ...
    def update_processes(self):
        ...
        for proc in psutil.process_iter(["pid", "name", "memory_percent", "memory_info"]):
            try:
                ...
                processes.append((info["pid"], info["name"] or "Unknown", info["memory_percent"] or 0.0, mem_mb))
        ...
        def _update_tree():
            ...
            for pid, name, mem_pct, mem_mb in top_processes:
                self.processes_tree.insert(
                    "", tk.END, values=(pid, name[:40] if len(name) > 40 else name, f"{mem_pct:.1f}", f"{mem_mb:.1f}")
                )
    ...
    def kill_selected_process(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Access Denied", f"Permission denied to kill '{name}' (PID: {pid}).\nTry running with sudo."
            )
        except Exception as e:
    ...
    def full_optimization(self):
        ...
        try:
            user_cache = RAMOptimizerDashboard._get_user_cache_path()
            commands = [f"sudo rm -rf {user_cache}/*", "sudo purge"]
...
class RAMOptimizerMenuBar(rumps.App):
    ...
    def update_memory(self, sender):
        ...
        try:
            ...
            if self.auto_optimize_enabled and mem.percent > self.auto_optimize_threshold and not self._is_on_battery():
                threading.Thread(target=self.auto_optimize_memory, daemon=True).start()
    ...
    def _clear_caches(self):
        ...
        try:
            user_cache = RAMOptimizerDashboard._get_user_cache_path()
            commands = [f"sudo rm -rf {user_cache}/*", "sudo rm -rf /Library/Caches/*"]
    ...
    def toggle_schedule(self, sender):
        ...
        rumps.notification(
            ...
            message=(
                f"Scheduled optimization "
                f"{'enabled' if self.schedule_enabled else 'disabled'} "
                f"(every {self.schedule_interval_minutes} min)"
            ),
        )
...
<dict>
    ...
    <false/>
            ...
            try:
                ...
                rumps.notification(
                    title="RAM Optimizer", subtitle="Error", message=f"Failed to create launch agent: {str(e)}"
                )
